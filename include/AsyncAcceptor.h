...
:
    Context_(socket.getContext()), AcceptorSocket(std::forward<AsyncPlatformSocket>(socket)) {
        auto status = AcceptorSocket.getsockname([&](const SocketAddress &addr) {
            AddressFamily_ = addr.getFamily();
        });
...
#else
        auto &context = Context_.getWriteContext(listensock);
        INTERNAL::setup(context, Context_.PendingIO, IO_OPERATION::IoAccept, 0, nullptr, callback);
        auto handle = ::accept4(listensock.value, NULL, NULL, SOCK_NONBLOCK);
