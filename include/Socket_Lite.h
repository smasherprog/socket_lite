...
enum class LingerOptions
{
    LINGER_OFF, LINGER_ON
};
enum class SockOptStatus
{
    ENABLED, DISABLED
};
struct LingerOption {
...
template <> struct setsockopt_factory_impl<SocketOptions::O_DEBUG> {
    static auto setsockopt_(PlatformSocket handle, SockOptStatus b) {
        return setsockopt_O_DEBUG(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_BROADCAST> {
    static auto setsockopt_(PlatformSocket handle, SockOptStatus b) {
        return setsockopt_O_BROADCAST(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_REUSEADDR> {
    static auto setsockopt_(PlatformSocket handle, SockOptStatus b) {
        return setsockopt_O_REUSEADDR(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_KEEPALIVE> {
    static auto setsockopt_(PlatformSocket handle, SockOptStatus b) {
        return setsockopt_O_KEEPALIVE(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_LINGER> {
    static auto setsockopt_(PlatformSocket handle, LingerOption b) {
        return setsockopt_O_LINGER(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_OOBINLINE> {
    static auto setsockopt_(PlatformSocket handle, SockOptStatus b) {
        return setsockopt_O_OOBINLINE(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_EXCLUSIVEADDRUSE> {
    static auto setsockopt_(PlatformSocket handle, SockOptStatus b) {
        return setsockopt_O_EXCLUSIVEADDRUSE(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_SNDBUF> {
    static auto setsockopt_(PlatformSocket handle, int b) {
        return setsockopt_O_SNDBUF(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_RCVBUF> {
    static auto setsockopt_(PlatformSocket handle, int b) {
        return setsockopt_O_RCVBUF(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_RCVTIMEO> {
    static auto setsockopt_(PlatformSocket handle, std::chrono::seconds b) {
        return setsockopt_O_RCVTIMEO(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_SNDTIMEO> {
    static auto setsockopt_(PlatformSocket handle, std::chrono::seconds b) {
        return setsockopt_O_SNDTIMEO(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_NODELAY> {
    static auto setsockopt_(PlatformSocket handle, SockOptStatus b) {
        return setsockopt_O_NODELAY(handle, b);
    }
};
template <> struct setsockopt_factory_impl<SocketOptions::O_BLOCKING> {
    static auto setsockopt_(PlatformSocket handle, Blocking_Options b) {
        return setsockopt_O_BLOCKING(handle, b);
    }
};

} // namespace INTERNAL
template <SocketOptions SO> auto getsockopt(PlatformSocket handle)
{
...
public:
    ...
    virtual void close();
    PlatformSocket get_handle() const {
        return handle;
    }
