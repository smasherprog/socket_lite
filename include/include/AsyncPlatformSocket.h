...
namespace SL::NET {
    ...
    public:
        ...
        static auto Create(const AddressFamily &family, Context &context)
        {
            int typ = SOCK_STREAM;
...
#if _WIN32
            u_long iMode = 1;
            ioctlsocket(handle.Handle_.value, FIONBIO, &iMode);
        ...
        template <class SOCKETHANDLERTYPE> static void connect(AsyncPlatformSocket &socket, SocketAddress &address, const SOCKETHANDLERTYPE &handler)
        {
            ...
            socket.Handle_.value = ::socket(address.getFamily(), SOCK_STREAM, 0);
            if (INTERNAL::win32Bind(address.getFamily(), socket.Handle_.value) == SOCKET_ERROR) {
