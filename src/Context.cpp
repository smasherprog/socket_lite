...
namespace SL {
    namespace NET {
        ...
        {
            ContextImpl_.ThreadCount_ = threadcount;
#if _WIN32
            ...
            DWORD bytes = 0;
            WSAIoctl(handle, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid), &ContextImpl_.ConnectEx_, sizeof(ContextImpl_.ConnectEx_), &bytes, NULL, NULL);
            closesocket(handle);
        ...
        void Context::run()
        {
            ContextImpl_.Threads.reserve(ContextImpl_.ThreadCount_.value);
            for (auto i = 0; i < ContextImpl_.ThreadCount_.value; i++) {
