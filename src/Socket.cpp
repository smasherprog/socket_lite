...
namespace SL {
    namespace NET {
        ...
        void continue_connect(bool success, Win_IO_Connect_Context *context)
        {
            ...
            Socket sock(*context->Context_, std::move(context->Socket_));
            delete context;
            if (success && ::setsockopt(sock.Handle().Handle().value, SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT, 0, 0) != SOCKET_ERROR) {
                if (h) {
                    h(StatusCode::SC_SUCCESS, std::move(sock));
                }
            }
            else if (h) {
                h(TranslateError(), Socket(*context->Context_));
            }
        }
        ...
        void connect(Context& c, sockaddr &address, std::function<void(StatusCode, Socket&&)> &&handler)
        { 
            PlatformSocket handle(Family(address));
            BindSocket(handle.Handle().value, Family(address));

            if (CreateIoCompletionPort((HANDLE)handle.Handle().value, c.ContextImpl_.IOCPHandle, NULL, NULL) == NULL) {
                return handler(TranslateError(), Socket(c));
            }
